...
if [ -z "$RESTOREBIN$DOWNLOADONLY" ]; then
    ...
    # If no prefix is set and the crouton partition is not being used, check that /usr/local/chroots ($CHROOTS) is a
    ...
    if [ -z "$PREFIXSET" -a ! -h "$CHROOTS" -a "$CHROOTS" != "$MOUNTPOINT/chroots" ] \
